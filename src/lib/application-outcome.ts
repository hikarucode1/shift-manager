...
export function applicationOutcome(
   ...
   * 教室長の記録 (#215)。
   *
   * ⚠️ **この判定は 2 つの仕組みに依存している。**
   *
   * 1. `withdrawApplication` のガード —「承認済みで自分が採用された応募は
   *    取り下げ不可」。将来「決まったが行けなくなった」を取り下げで表せる
   *    ように緩めると、**選ばれた講師が黙って「教室長が記録」に変わる**。
   * 2. `decideSwapRequest` が承認時に `swap_requests` を `FOR UPDATE` で
   *    ロックすること (#249)。**1 だけでは足りなかった** — ロックが無いと
   *    「承認が応募行を読む → 取り下げが commit → 承認が commit」が通り、
   *    1 のガードは取り下げ時点でまだ pending なので発動しない。
   */
  applied: boolean,
): ApplicationOutcome {
