...
describe("applicationOutcome", () => {
  it("承認は「自分が選ばれたか」で意味が正反対になる", () => {
    ...
    expect(applicationOutcome("approved", true, true)).toBe("chosen");
    expect(applicationOutcome("approved", false, true)).toBe("not-chosen");
  });

  it("却下は chosen / wasApproved に依らない", () => {
    expect(applicationOutcome("rejected", false, false)).toBe("rejected");
    expect(applicationOutcome("rejected", true, true)).toBe("rejected");
  });

  it("取り消しは「一度承認されたか」で分ける", () => {
    // 落選者から見ると「募集が取り下げられた」と「決まった代講が後から
    // 取り消された」は別の出来事
    expect(applicationOutcome("cancelled", false, false)).toBe("withdrawn");
    expect(applicationOutcome("cancelled", false, true)).toBe(
      "cancelled-after-approval",
    );
    expect(applicationOutcome("cancelled", true, true)).toBe(
      "cancelled-after-approval",
    );
  });
