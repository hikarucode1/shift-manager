...
export function RecordSubstitutionForm({ today }: { today: string }) {
  ...
  function submit() {
    ...
    startSaving(async () => {
      ...
      if (res.pendingSwap) {
        parts.push(
          // #253: 申請者と応募者にも「承認できなくなった」を通知済み。
          // それを書かないと、教室長は講師が待たされ続けていると思って急ぐ
          "未処理の交代申請が残っています。担当が変わったため承認できないので、「未対応」タブで却下（教室長が出した代理募集なら取り下げ）してください。申請者と応募者には承認できない旨を通知済みです。",
        );
      }
