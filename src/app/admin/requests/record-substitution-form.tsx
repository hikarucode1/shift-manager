...
export function RecordSubstitutionForm({ today }: { today: string }) {
  ...
  function submit() {
    ...
    startSaving(async () => {
      ...
      if (res.pendingSwap) {
        parts.push(
          // #253: 申請者と「現時点の」応募者には通知済み。
          // ⚠️ **「応募者に通知済み」と言い切らない。** 募集は pending のまま
          // 残るので、閉じるまでは新しい応募が入りうる。その講師には通知が
          // 届かないので、教室長が閉じる動機を消してはいけない
          "未処理の交代申請が残っています。担当が変わったため承認できないので、「未対応」タブで却下（教室長が出した代理募集なら取り下げ）してください。申請者と現時点の応募者には通知済みですが、閉じるまでは新しい応募が入る場合があります。",
        );
      }
