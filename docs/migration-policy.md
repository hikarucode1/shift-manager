...
| 0034 | `absence_requests.created_by` 追加 (#217) | 非破壊 (nullable 列 + FK 追加のみ) | 任意だが**コード deploy より先に適用**。教室長の代理登録 (`createAbsenceOnBehalf`) と 講師本人の申請の insert がこの列に書くため、列が無い状態でコードが動くと insert が 落ちて欠勤を登録できない。既存行は null (本人申告か代理登録か不明) のままでよい |
